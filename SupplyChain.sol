...
contract MyContract{
    ...
    function addProduct(string memory _pname, int _price, int _quantity,string memory _regname, string memory _maddress ) public addProductAuth {
        totalProduct += 1;
        products[totalProduct] = product(totalProduct,_price, _quantity, _pname,_regname,_maddress, msg.sender);
    }
    ...
    function getProductById(int _pid) view public returns (int,string memory, int, int ,string memory) {
        require(_pid <= totalProduct);
        return (products[_pid].id , products[_pid].product_name, products[_pid].price, products[_pid].quantity, products[_pid].maddress);
    }
    
    function placeOrder(int _pid,int _quantity,string memory _cname, string memory _daddress ) public {
